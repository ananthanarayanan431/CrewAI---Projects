...
class MarketingAnalysisTasks:
	def product_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			expected_output="A detailed report outlining the product's key selling points, market appeal, unique features, and suggestions for enhancement or positioning.",
			agent=agent
		)
	...
	def competitor_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A comprehensive report detailing the top 3 competitors' strategies, market positioning, customer perception, and a comparison with the provided product website.",
		)
	...
	def campaign_development(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			expected_output="A set of strategic and creative content ideas for a marketing campaign that effectively engages the target audience, along with comprehensive context about the product and customer details.",
			agent=agent,
	...
	def instagram_ad_copy(self, agent):
		return Task(description=dedent("""\
			...
			"""),
			expected_output="Three distinct options for Instagram ad copy that are engaging, concise, and aligned with the product's marketing strategy, designed to resonate with the target audience and encourage action.",
			agent=agent
		)
	...
	def take_photograph_task(self, agent, copy, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="Three descriptions of potential Instagram photographs, each described in a paragraph and aligned with the provided examples, focusing on creative and engaging imagery that captures the audience's attention.",
		)
	...
	def review_photo(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			expected_output="Three reviewed options of photographs, each described in a paragraph, ensuring alignment with product goals and following the provided examples.",
			agent=agent,
		)